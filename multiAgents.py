    ...
    def evaluationFunction(self, currentGameState: GameState, action):
        ...
        for action in currentGameState.getLegalActions(0):
            if currentGameState.generateSuccessor(0, action).getPacmanPosition() in newFood.asList():
                return successorGameState.getScore() - (maxDistanceToFoods + minDistanceToFoods + 500) + \
                       minDistanceToGhosts
        ...
        def min_value(gameState, indexAgent, depth, move):
            ...
            if indexAgent + 1 == gameState.getNumAgents():
                for action in gameState.getLegalActions(indexAgent):
                    v2, a2 = max_value(gameState.generateSuccessor(indexAgent, action), 0, depth + 1, move)
                    if v2 < v:
                        v, move = v2, action
            else:
                for action in gameState.getLegalActions(indexAgent):
                    v2, a2 = min_value(gameState.generateSuccessor(indexAgent, action), indexAgent + 1, depth, move)
                    if v2 < v:
                        v, move = v2, action
            return v, move
...
class AlphaBetaAgent(MultiAgentSearchAgent):
    ...
    def getAction(self, gameState: GameState):
        ...
        def max_value(gameState, pacman, depth, move, alfa, beta):
            ...
            for action in gameState.getLegalActions(pacman):
                v2, a2 = min_value(gameState.generateSuccessor(pacman, action), 1, depth, move, alfa, beta)
                if v2 > v:
                    v, move = v2, action
                    alfa = max(alfa, v)
                if v > beta:
        ...
        def min_value(gameState, indexAgent, depth, move, alfa, beta):
            ...
            if indexAgent + 1 == gameState.getNumAgents():
                for action in gameState.getLegalActions(indexAgent):
                    v2, a2 = max_value(gameState.generateSuccessor(indexAgent, action), 0, depth + 1, move, alfa, beta)
                    if v2 < v:
                        v, move = v2, action
                        beta = min(beta, v)
                    if v < alfa:
